...
<title>상품 목록 | smartconvergence</title>
...
var FN = 
{
			...
			$.ajax( "/pc/product/productList",{
				...
				timeout : 20000, //제한시간 지정
			 	...
			 	success: function(data){
			 		...
			 		var products = data.response.body.products;
			 		
			 		pageInfo = data.response.body.pageInfo;

			 		//기존 row들 삭제
			 		$("#productList .row").detach();
			 		
			 		//데이터가 없으면
			 		if(pageInfo.resultCount == 0)
			 		{
			 			$("#productList .nodata").css("display", "");
			 		...
			 		//데이터가 하나라도 있으면
		 			$("#productList .nodata").css("display", "none");
		 			$(".paging").css("display", "");
			 		...
			 		$(products).each(function(idx) {
			 			var row = $("#productList .src_row").clone();
			 			
			 			$(row).removeClass("src_row");
			 			$(row).addClass("row");

			 			$(row).find(".topRowCnt").text(topRowCnt-idx);
			 			$(row).find(".productName").text(products[idx].itemName);
			 			$(row).find(".branchName").text(products[idx].branchName);
			 			$(row).find(".createUser").text(products[idx].createName);
			 			$(row).find(".updateDate").text(products[idx].updateDttm);
			 			//display 보이게 하고
			 			$(row).css("display", "");
			 			
			 			$(row).children().each(function(){
			 				$(this).click(function(){
			 					FN.detail(products[idx].itemId);
			 				});
			 			});
			 	...
			 	error: function(result, status){
			 		var  statusStr = "/pc/product/productList :"+status;
				 	// session time out
		...
		detail:function(productId) {
			location.href = "/pc/product/productDetailPage?productId="+productId;
		}
}
...
<div id="wrap">
	...
	<div id="container"> 
		<div class="contents">
		<h2><span class="titel-text-sm">상품 관리 > </span>상품 목록</h2>
	  <!-- CONTENTS -->
		...
		<table class="mytable">
			...
			<tbody id="productList">
				...
				<tr class="src_row" style="display:none">
					...
					<td><a class="productName"></a></td>
					<td><a class="branchName"></a></td>
					<td><a class="createUser"></a></td>
					<td><a class="updateDate"></a></td>
				</tr>
			</tbody>
		</table>
