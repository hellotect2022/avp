...
<title>AR 목록 | smartconvergence</title>
...
var FN = 
{
			...
			$.ajax( "/pc/ar/arList",{
				...
				timeout : 20000, //제한시간 지정
			 	...
			 	success: function(data){
			 		...
			 		var ars = data.response.body.ars;
			 		console.log(ars)
			 		...
			 		//데이터가 하나라도 있으면
		 			$("#arList .nodata").css("display", "none");
		 			$(".paging").css("display", "");
		 			$(".table_box input[type=checkbox]").prop('checked',false);

			 		var topRowCnt = pageInfo.totalCount - ((pageInfo.page-1) * pageInfo.size);

			 		$(ars).each(function(idx) {
			 			var row = $("#arList .src_row").clone();
			 			$(row).removeClass("src_row");
			 			$(row).addClass("row");
			 			 
			 			$(row).find(".topRowCnt").text(topRowCnt-idx);
			 			$(row).find(".arName").text(ars[idx].arName);
			 			$(row).find(".itemName").text(ars[idx].itemName);
			 			$(row).find(".createDate").text(ars[idx].createDate);
			 			//display 보이게 하고
			 			$(row).css("display", "");
			 			...
			 			$(row).children().each(function(){
			 				if(!$(this).find("input").hasClass("listChk")){
				 				$(this).click(function(){
				 					FN.detail(ars[idx].arId);
				 				});
				 				$(".src_row a").hover(function() {
			 	...
			 	error: function(result, status){
					var  statusStr = "AR 목록 가져오기 :"+status;
				 	// session time out
		...
		detail:function(arId) {
			location.href = "/pc/ar/arDetailPage?arId="+arId;
		}
}
...
<div id="wrap">
	...
	<div id="container"> 
		<div class="contents">
			<h2><span class="titel-text-sm">AR 관리 > </span>AR 목록</h2>
			<!-- CONTENTS -->
