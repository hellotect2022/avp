...
var FN = 
{
		...
		{
			FN.dialogShow();
			$.ajax( "/pc/member/memberAuthApplyList",{
			 	type : "POST", //"POST", "GET"
				dataType :  "json", //전송받을 데이터의 타입
				data : JSON.stringify({
					"pageInfo":{
...
// 					"period":period
				...
				timeout : 20000, //제한시간 지정
			 	...
			 	success: function(data){
			 		...
			 		$(members).each(function(idx) {
			 			...
			 			else if("SELLER"	==	members[idx].userType)
			 			{
			 				str = "판매자"
			 			}
			 	...
			 	error: function(result, status){
			 		var  statusStr = "/pc/member/memberAuthApplyList :"+status;
					 	// session time out
		...
		detail:function(userId) {
			location.href = "/pc/member/memberAuthApplyDetailPage?userId="+userId;
		}
}
...
<div id="wrap">
	<div id="container">
		<div class="contents">
			<h2><span class="titel-text-sm">멤버 관리 > </span>권한신청대기</h2>
			<!-- CONTENTS -->
		...
		<table class="mytable">
			...
			<tbody id="memberList">
				<tr class="nodata" style="display:none">
					<td class="brln" colspan="6">- 데이터가 없습니다 -</td>
				</tr>
