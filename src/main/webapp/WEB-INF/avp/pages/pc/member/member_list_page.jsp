...
<title>멤버목록 | smartconvergence</title>
...
// 					"period":period
				...
				timeout : 20000, //제한시간 지정
			 	...
			 	success: function(data){
			 		...
			 		$(members).each(function(idx) {
			 			...
			 			$(row).find(".userType").text(members[idx].userType);
			 			$(row).find(".createDttm").text(members[idx].createDttm);
			 			//display 보이게 하고
			 			$(row).css("display", "");
			 	...
			 	error: function(result, status){
			 		var  statusStr = "/pc/member/memberList :"+status;
				 	// session time out
		...
		detail : function(userId) {
			location.href = "/pc/member/memberDetailPage?userId=" + userId;
		}
}
...
<div id="container"> 
    <div class="contents">
		<h2><span class="titel-text-sm">멤버 관리 > </span>멤버목록 </h2>
	...
	<table class="mytable">
		<caption>멤버목록</caption>
        <colgroup>
            <col width="20%"/>			<!-- NO -->
            <col width="25%"/>			<!-- NickName -->
            <col width="25%"/>			<!-- Grade -->
            <col width="30%"/>			<!-- CreateDate -->
        </colgroup>
        <thead>
            <tr>
                <th scope="col">NO</th>
