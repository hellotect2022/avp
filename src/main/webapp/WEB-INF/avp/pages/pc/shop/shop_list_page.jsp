...
<title>지점 목록 | smartconvergence</title>
...
var FN = 
{
			...
			$.ajax( "/pc/shop/shopList",{
				...
				timeout : 20000, //제한시간 지정
			 	...
			 	success: function(data){
			 		...
			 		var branches = data.response.body.shops;
			 		...
			 		{
			 			$("#shopList .nodata").css("display", "");
			 			$(".paging").css("display", "none");
				 		$(".backLayer").hide();			 		
			 			return;
			 		}
			 		//데이터가 하나라도 있으면
		 			$("#shopList .nodata").css("display", "none");
		 			$(".paging").css("display", "");
		 			$(".table_box input[type=checkbox]").prop('checked',false);

			 		var topRowCnt = pageInfo.totalCount - ((pageInfo.page-1) * pageInfo.size);

			 		$(branches).each(function(idx) {
			 			var row = $("#shopList .src_row").clone();
			 			$(row).removeClass("src_row");
			 			$(row).addClass("row");
			 			 
			 			$(row).find(".topRowCnt").text(topRowCnt-idx);
			 			$(row).find(".branchName").text(branches[idx].branchName);
			 			$(row).find(".userCount").text(branches[idx].userCount);
			 			$(row).find(".updateDate").text(branches[idx].updateDate);
			 			//display 보이게 하고
			 			$(row).css("display", "");
			 			...
			 			$(row).children().each(function(){
			 				if(!$(this).find("input").hasClass("listChk")){
				 				$(this).click(function(){
				 					FN.detail(branches[idx].branchId);
				 				});
			 				}
			 			});
		...
		detail : function(branchId) {
			location.href = "/pc/shop/shopDetailPage?branchId="+branchId
		}
}
...
<div id="wrap">
	...
	<div id="container"> 
		...
		<div class="contents">
			<h2><span class="titel-text-sm">소속 관리 > </span>지점 목록</h2>
		  <!-- CONTENTS -->
