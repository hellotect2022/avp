...
<title>디바이스 목록 | smartconvergence</title>
...
var FN = 
{
			...
			$.ajax( "/pc/device/deviceList",{
				...
				timeout : 20000, //제한시간 지정
			 	...
			 	success: function(data){
			 		...
			 		//기존 row들 삭제
			 		$("#deviceList .row").detach();
			 		
			 		//데이터가 없으면
			 		if(pageInfo.resultCount == 0)
			 		{
			 			$("#deviceList .nodata").css("display", "");
			 			$(".paging").css("display", "none");
				 		$(".backLayer").hide();			 		
			 			return;
			 		}
			 		//데이터가 하나라도 있으면
		 			$("#deviceList .nodata").css("display", "none");
		 			$(".paging").css("display", "");
		 			$(".table_box input[type=checkbox]").prop('checked',false);

			 		var topRowCnt = pageInfo.totalCount - ((pageInfo.page-1) * pageInfo.size);

			 		$(devices).each(function(idx) {
			 			var row = $("#deviceList .src_row").clone();
			 			$(row).removeClass("src_row");
			 			$(row).addClass("row");
			 			 
			 			$(row).find(".topRowCnt").text(topRowCnt-idx);
			 			$(row).find(".deviceName").text(devices[idx].deviceName);
			 			$(row).find(".serial").text(devices[idx].serial);
			 			$(row).find(".updateDttm").text(devices[idx].updateDttm);
			 			...
			 			$(row).children().each(function(){
			 				if(!$(this).find("input").hasClass("listChk")){
				 				$(this).click(function(){
				 					FN.detail(devices[idx].deviceId);
				 				});
			 				}
			 			});
			 	...
			 	error: function(result, status){
			 		var  statusStr = "/pc/device/deviceList :"+status;
				 	// session time out
		...
		detail : function(deviceId) {
			location.href = "/pc/device/deviceDetailPage?deviceId=" + deviceId;
		}
}
...
<div id="wrap">
	...
	<div id="container"> 
		...
		<div class="contents">
			<h2><span class="titel-text-sm">디바이스 관리 > </span>디바이스 목록</h2>
		  <!-- CONTENTS -->
		...
		<table class="mytable">
			...
			<tbody id="deviceList">
				...
				<tr class="src_row" style="display:none" align="center">
					...
					<td><a class="deviceName"></a></td>
					<td><a class="serial"></a></td>
